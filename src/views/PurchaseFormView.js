...
export class PurchaseFormView {
  ...
  open(suppliers, products, { onSubmit, onCancel }) {
    ...
    body.className = 'purchase-form'
    body.append(
      this._createSupplierSection(suppliers),
      this._createProductSection(),
      this._createCartSection(),
      this._createNotesSection(),
      this._createActionsSection()
    )

    this._modal = new Modal({ title: 'Nueva Compra', content: body, size: 'xl' })
    this._modal.open()
  }
  ...
  _createSupplierSection(suppliers) {
    const group = document.createElement('div')
    group.className = 'purchase-form__section'

    const label = document.createElement('label')
    label.className = 'purchase-form__label'
    label.textContent = 'Proveedor'

    this._supplierSelect = document.createElement('select')
    this._supplierSelect.className = 'purchase-form__select'

    const empty = document.createElement('option')
    empty.value = ''
    empty.textContent = 'Seleccione un proveedor...'
    this._supplierSelect.appendChild(empty)

    suppliers.forEach(s => {
      const opt = document.createElement('option')
      opt.value = s.id
      opt.textContent = `${s.name} (${s.documentId})`
      this._supplierSelect.appendChild(opt)
    })

    group.append(label, this._supplierSelect)
    return group
  }
  ...
  _createProductSection() {
    ...
    row.className = 'purchase-form__product-row'

    this._productSelect = document.createElement('select')
    this._productSelect.className = 'purchase-form__select purchase-form__select--product'
    ...
    this._products.forEach(p => {
      const opt = document.createElement('option')
      opt.value = p.id
      opt.textContent = `[${p.code}] ${p.name} — ${formatCurrency(p.purchasePrice)}`
      this._productSelect.appendChild(opt)
    })

    this._qtyInput = document.createElement('input')
    Object.assign(this._qtyInput, {
      className: 'purchase-form__input purchase-form__input--qty',
      type: 'number', min: 1, step: 1, value: '1', placeholder: 'Cant.'
    })

    this._priceInput = document.createElement('input')
    Object.assign(this._priceInput, {
      className: 'purchase-form__input purchase-form__input--price',
    ...
    row.append(this._productSelect, this._qtyInput, this._priceInput, addBtn)
  ...
  _createCartSection() {
    ...
    wrapper.className = 'purchase-form__cart-wrapper'
    ...
    this._totalsDiv.className = 'purchase-form__totals'
    this._totalsDiv.append(
      createRow('Subtotal:', 'purchase-subtotal'),
      createRow(`Impuesto (${(getTaxRate() * 100).toFixed(0)}%):`, 'purchase-tax'),
      createRow('Total:', 'purchase-total', 'purchase-form__total-row purchase-form__total-row--grand')
    )
  ...
  _createNotesSection() {
    ...
    this._notesInput.className = 'purchase-form__textarea'
  ...
  _createActionsSection() {
    ...
    actions.className = 'purchase-form__actions'
    ...
    this._saveBtn.textContent = 'Guardar Compra'
  ...
  _addProduct() {
    ...
    const unitPrice = Number(this._priceInput.value) || (product.purchasePrice || 0)
  ...
  _renderCart() {
    ...
    if (this._cart.length === 0) {
      ...
      td.className = 'table__empty'; td.colSpan = 5; td.textContent = 'Agregue productos a la compra'
    ...
    for (let i = 0; i < this._cart.length; i++) {
      ...
      row.children[1].textContent = String(item.quantity)
      row.children[2].textContent = formatCurrency(item.unitPrice)
      row.children[3].textContent = formatCurrency(item.subtotal)
      row.children[3].className = 'table__td text-semibold'
  ...
  _submit() {
    ...
    const supplierId = this._supplierSelect.value ? Number(this._supplierSelect.value) : null
    let supplierName = ''
    if (supplierId && this._supplierSelect.selectedOptions[0]) {
      supplierName = this._supplierSelect.selectedOptions[0].textContent.split(' (')[0]
    }

    if (this._onSubmit) {
      this._onSubmit({
        purchaseData: { supplierId, supplierName, notes: this._notesInput.value },
  ...
  showSaving() {
    if (this._saveBtn) { this._saveBtn.disabled = true; this._saveBtn.textContent = 'Guardando...' }
  }

  hideSaving() {
    if (this._saveBtn) { this._saveBtn.disabled = false; this._saveBtn.textContent = 'Guardar Compra' }
  }
}
